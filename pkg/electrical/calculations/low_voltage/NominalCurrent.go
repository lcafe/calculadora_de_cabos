...
func NominalCurrent(power float64, voltage float64, powerFactor float64, demandFactor float64, efficiency float64) (float64, error) {
	...
	if math.IsNaN(powerFactor) || math.IsInf(powerFactor, 0) {
		return 0, fmt.Errorf("Fator de potência inválida: valor não numérico ou infinito")
	}

	if powerFactor <= 0 {
		return 0, fmt.Errorf(
			"Fator de potência inválida: deve ser maior que zero (valor informado: %.2f)",
			powerFactor,
		)
	}
	...
	if powerFactor > 1 {
		return 0, fmt.Errorf(
			"Fator de potência inválida: deve ser menor que um (valor informado: %.2f)",
			powerFactor,
		)
	}

	if math.IsNaN(demandFactor) || math.IsInf(demandFactor, 0) {
		return 0, fmt.Errorf("Fator de demanda inválida: valor não numérico ou infinito")
	}
	...
	if demandFactor > 1 {
		return 0, fmt.Errorf(
			"Fator de demanda inválido: deve ser menor que um (valor informado: %.2f)",
			demandFactor,
		)
	}
